...
\begin{quote}
$M=$ ``对于输入 $\langle A\rangle$：

\medskip
\noindent
1. 利用正则语言与上下文无关语言求交的有效构造，构造一个 CFG $G_A$，满足
\[
    L(G_A)=L(A)\cap C_{=}.
\]
...
若 $M$ 接受，则存在某个被 $A$ 接受且满足
\[
    \#_0=\#_1
\]
的字符串；若 $M$ 拒绝，则不存在这样的字符串。于是
\[
    \mathrm{BAL}_{\mathrm{DFA}}
    =\{\langle A\rangle\mid A\text{ 是 DFA 且接受某个 }0,1\text{ 个数相同的串}\}
\]
是可判定的。$\blacksquare$
