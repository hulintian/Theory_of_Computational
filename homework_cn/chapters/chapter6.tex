...
\[
    A\le_T B
\]
表示 $A$ 可相对于 $B$ 判定。也就是说，存在一台带预言机 $B$ 的图灵机 $M^B$ 判定 $A$。直观地说，如果我们有能力回答
\[
    x\in B?
\]
这样的询问，那么就能判定
\[
    w\in A?
\]
因此方向是：假设能判定右边的语言 $B$，就能判定左边的语言 $A$。
...
\[
    ...
    B \le_T C.
\]
由定义，存在带预言机 $B$ 的图灵机 $M^B$ 判定 $A$，也存在带预言机 $C$ 的图灵机 $N^C$ 判定 $B$。

\medskip
构造一台带预言机 $C$ 的图灵机 $T^C$ 来判定 $A$。在输入 $w$ 上，$T^C$ 模拟 $M^B(w)$ 的运行。每当 $M^B$ 向其 $B$-预言机询问某个字符串 $x$ 是否属于 $B$ 时，$T^C$ 不直接询问 $B$，而是在输入 $x$ 上运行 $N^C$。由于 $N^C$ 判定 $B$，所以它会正确判定
\[
    x\in B
\]
是否成立。若 $N^C$ 接受，则 $T^C$ 把这次预言机询问回答为“是”；若 $N^C$ 拒绝，则回答为“否”。除此之外，$T^C$ 完全照着 $M^B$ 的步骤运行。

\medskip
因为 $N^C$ 判定 $B$，所以 $T^C$ 对每次关于 $B$ 的预言机询问都能给出和真正的 $B$-预言机相同的答案。于是 $T^C$ 的整个计算与 $M^B$ 的计算完全一致。对任意输入 $w$，
\[
    T^C \text{ 接受 } w
    \iff
    M^B \text{ 接受 } w
    ...
    w\in A.
\]
而且 $M^B$ 是判定器，每次调用的 $N^C$ 也是判定器，所以 $T^C$ 也总会停机。故 $T^C$ 判定 $A$。这说明存在一台带预言机 $C$ 的图灵机判定 $A$，从而
\[
    A \le_T C.
\]
