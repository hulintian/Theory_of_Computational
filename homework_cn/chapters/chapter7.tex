...
\]
表示每门考试所在的时段。验证时，对每个学生枚举他参加的考试对，
检查这些考试是否被安排到不同的时段。检查次数至多为登记关系规模的平方，
因此仍是输入规模的多项式，所以
\[
...
h=3.
\]
该构造的规模也是多项式的。对输入图 $G=(V,E)$，考试数为 $|V|$，
学生数为 $|E|$，每个学生只登记两门考试，所以登记关系一共有 $2|E|$ 项，
并且 $h=3$ 是常数。因此输出实例大小为 $O(|V|+|E|)$，
按顶点和边扫描即可构造完成，所需时间为多项式。
...
\end{itemize}
该构造只把原图中的每个顶点保留一次，额外加入一个中心顶点 $c$，
并且对每条边加入两个新顶点 $a_e,b_e$。因此
\[
|V(G')|=|V|+1+2|E|.
\]
边也只有三类：$c$ 与每个原顶点相连，$a_e$ 与边 $e$ 的两个端点相连，
以及 $a_e$ 与 $b_e$ 相连，所以
\[
|E(G')|=|V|+3|E|.
\]
所有标号和边都可以在扫描 $V$ 与 $E$ 时直接写出，故构造可在多项式时间内完成。
